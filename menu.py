...
from functions import process_text, load_image
...
def home():
    ...
    title_size = 130
    TwoD, TwoD_rect = process_text("TwoD", HW, HH - 230, Colors.WHITE, title_size,
                                   title_font)
    Battlegrounds, Battlegrounds_rect = process_text("Battlegrounds", HW, HH - 130,
                                                     Colors.WHITE, title_size, title_font)

    play_button = Button("graphics/play_button", HW, HH + 200)
    play_text, play_text_rect = process_text("PLAY", HW, HH + 200, Colors.WHITE, 50,
                                             "ABITE")
    ...
    while not (((play_button.hover(mouse.position()) and mouse.pressed()) or keys[
        K_RETURN]) and len(name) > 0):
        keys = pygame.key.get_pressed()

        name = input.read()
        if len(name) == 0:
            nickname, nickname_rect = process_text("Type your nickname", HW, H - 230,
                                                   Colors.LIGHT_GREY, 30)
        else:
            nickname, nickname_rect = process_text(name, HW, H - 230, Colors.GREY, 20,
                                                   "m12")
        ...
        pygame.draw.rect(SCREEN, Colors.WHITE, (HW - 150, H - 260, 300, 60))
        ...
        input.show_errors(SCREEN, HW, H - 290, Colors.RED)
