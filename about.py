...
def read_credits(SCREEN, HW):
    ...
    for line in credits_text:
        if line[0] == "*":
            ...
            line_size = 300
            pygame.draw.line(SCREEN, (255, 255, 255), (HW - line_size, y + 15),
                             (HW + line_size, y + 15))
        else:
...
def header(SCREEN, x, y):
    ...
    pygame.draw.line(SCREEN, (255, 255, 255), (0, y + 30),
                     (x + (150 * len(head)), y + 30))
    for text in head:
...
def credits(W, H):
    ...
    max_scroll = H
    while events(max_scroll, H) and not (
            back_button.hover(mouse.position()) and mouse.pressed()):
