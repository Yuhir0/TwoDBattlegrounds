...
def read_credits(SCREEN, HW):
    ...
    for i in range(len(credits_text)):
        endline = len(credits_text[i]) - 1
        if credits_text[i][0] == "*":
            text, text_rect = process_text(credits_text[i][1:endline], HW, y, (255,255,255), 30)
        else:
            text, text_rect = process_text(credits_text[i][:endline], HW, y, (255,255,255), 20)
...
def read_score(SCREEN, HW, H):
    ...
    if len(score_text) > 0:
        for i in range(len(score_text)):
            ...
            text, text_rect = process_text(str(i + 1) + ") " + score_text[i][:endline], HW, y, (255,255,255), 20)
            SCREEN.blit(text, text_rect)
            y += 40
    else:
...
def credits(W, H):
    ...
    max_scroll = H
    while events(max_scroll, H):
        max_scroll = read_credits(SCREEN, process_text, HW)
...
def score(W, H):
    ...
    max_scroll = H
    while events(max_scroll, H):
        max_scroll = read_score(SCREEN, process_text, HW, H)
        pygame.display.update()
        SCREEN.fill(BLACK)
        CLOCK.tick(FPS)
    posy = 30
